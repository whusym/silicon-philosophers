#!/usr/bin/env python3
"""
Evaluate Grok 4.6 via OpenRouter, mirroring 3_model_eval.py behavior.
...
OpenRouter model: x-ai/grok-4.6
...
def run_sync(data_dir: Path, output_dir: Path, limit: Optional[int]) -> None:
    ...
    for task in tqdm(tasks, desc="Grok 4.6 sync", unit="item"):
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="OpenRouter Grok 4.6 eval (mirrors 3_model_eval.py)"
    )
    p.add_argument(
        "--mode",
        choices=["sync", "batch"],
...
def main() -> None:
    args = parse_args()
    print("=" * 72)
    print("OpenRouter Grok 4.6 evaluation")
